import type { HeroProps } from "../interfaces";

export const heroData: HeroProps[] = [
    {
        backgroundClass:
            'bg-[url("/src/assets/hero/start_transformation.jpg")] bg-[position:top_center]',
        wrapperClass: "py-5 lg:py-25",
        title: "WELCOME TO TRAVIESAS",
        titleClass:
            "text-xl lg:text-6xl font-light tracking-tight text-white mb-4",
        subtitle:
            "Traviesas is the home of beautiful smiles. Discover a radiant and healthy smile in just two visits!",
        subtitleClass:
            "block max-w-10/12 lg:max-w-1/2 mt-4 mx-auto text-xl text-gray-200 font-light leading-relaxed",
        buttons: [
            {
                text: "Request Information",
                href: "#",
                className:
                    "relative bg-white text-gray-800 hover:bg-[#df3884] hover:text-white text-md rounded-full shadow-lg  after:content-none sm:after:content-[''] after:absolute after:top-1/2 after:left-full after:transform after:-translate-y-1/2 after:w-[2px] after:h-4 after:bg-gray-300 after:mx-2 max-w-[250px]",
            },
            {
                text: "Call (305) 826-8980",
                href: "tel:+13058268980",
                className:
                    "bg-[#6c757d] text-white hover:bg-[#5a6268] rounded-full shadow-lg max-w-[250px]",
            },
        ],
    },
    {
        backgroundClass:
            'bg-[url("/src/assets/hero/we_are_open.jpg")] bg-[position:top_center]',
        wrapperClass: "py-5 lg:py-55",
        title: "WE ARE OPEN!",
        titleClass:
            "text-xl lg:text-6xl font-light tracking-tight text-white mb-4",
        subtitle:
            "Traviesas is open and ready to welcome you. We follow all safety measures to protect our patients and staff. Book your appointment today.",
        subtitleClass:
            "block max-w-10/12 lg:max-w-1/2 mt-4 mx-auto text-xl text-gray-200 font-light leading-relaxed",
        buttons: [
            {
                text: "Book Appointment",
                href: "#",
                className:
                    "bg-white text-gray-800 hover:bg-[#df3884] hover:text-white hover:border-[#eb3588] hover:border text-md rounded-full shadow-lg border border-transparent max-w-[250px]",
            },
        ],
    },
    {
        backgroundClass:
            'bg-[url("/src/assets/hero/cg_smile_home.jpg")] bg-[position:top_center]',
        wrapperClass: "py-5 lg:py-44",
        title: "START YOUR SMILE TRANSFORMATION",
        titleClass:
            "text-xl lg:text-6xl font-light max-w-3/5 mx-auto tracking-tight text-white mb-4",
        subtitle:
            "The first step to achieving the smile you have always wanted starts with a consultation at Traviesas.",
        subtitleClass:
            "block max-w-10/12 lg:max-w-1/2 mt-4 mx-auto text-xl text-gray-200 font-light leading-relaxed",
        buttons: [
            {
                text: "Get Started",
                href: "#",
                className:
                    "bg-white text-gray-800 hover:bg-[#df3884] hover:text-white hover:border-[#eb3588] hover:border text-md rounded-full shadow-lg border border-transparent py-3 max-w-[250px]",
            },
        ],
    },
    {
        backgroundClass:
            'bg-[url("/src/assets/dentalServices/smile.jpg")] bg-[position:bottom_center]',
        wrapperClass: "py-5 lg:py-25",
        title: "Smile Makeover & Smile Design at Traviesas",
        titleClass:
            "text-xl lg:text-6xl max-w-10/12 lg:max-w-3/4 font-light tracking-tight text-white mb-4 mx-auto",
        subtitle:
            "Are you hiding your smile because of gapped, chipped, crooked, or misshapen teeth? At Traviesas, we help you regain your confidence. Contact us and start your transformation!",
        subtitleClass:
            "block max-w-10/12 lg:max-w-3/4 mt-4 mx-auto text-base text-gray-200 font-medium leading-relaxed",
        buttons: [
            {
                text: "Request Information",
                href: "#",
                className:
                    "relative bg-white text-gray-800 hover:bg-[#df3884] hover:text-white text-md rounded-full shadow-lg  after:content-none sm:after:content-[''] after:absolute after:top-1/2 after:left-full after:transform after:-translate-y-1/2 after:w-[2px] after:h-4 after:bg-gray-300 after:mx-2 max-w-[250px]",
            },
            {
                text: "Call (305) 826-8980",
                href: "tel:+13058268980",
                className:
                    "bg-[#6c757d] text-white hover:bg-[#5a6268] rounded-full shadow-lg max-w-[250px]",
            },
        ],
    },
    {
        backgroundClass:
            'bg-[url("/src/assets/hero/love.jpg")] bg-[position:bottom_center]',
        wrapperClass: "py-5 lg:py-30",
        title: "Love Your Smile!",
        titleClass:
            "text-xl lg:text-6xl font-light max-w-3/5 mx-auto tracking-tight text-white mb-4",
        subtitle:
            "Do you feel uncomfortable smiling because of your teeth? Would you like a smile that stands out for all the right reasons? Call Traviesas and book your personalized smile design consultation.",
        subtitleClass:
            "block max-w-10/12 lg:max-w-7/12 mt-4 mx-auto text-xl text-gray-200 font-light leading-relaxed",
        buttons: [
            {
                text: "Book Online Consultation",
                href: "#",
                className:
                    "bg-[#df3884] text-white hover:bg-white hover:text-gray-800 text-md rounded-full shadow-lg border border-transparent py-3 w-fit",
            },
        ],
    },
];
